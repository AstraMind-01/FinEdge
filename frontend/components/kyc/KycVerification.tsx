...
interface KycVerificationProps {
  documentsList?: Array<{ icon: any; title: string; status: string }>;
  onViewDocument?: (title: string, status: string) => void;
  onUpdateDocument?: (title: string) => void;
}

export default function KycVerification({
  documentsList: propDocs,
  onViewDocument,
  onUpdateDocument
  ...
  const documents = propDocs || defaultDocs;
              ...
              <div className="col-span-6 md:col-span-4 flex items-center justify-end gap-4">
                {doc.status === 'Verified' ? (
                  <>
                    <button 
                      type="button"
                      onClick={() => onViewDocument?.(doc.title, doc.status)} 
                    ...
                    <button 
                      type="button"
                      onClick={() => onUpdateDocument?.(doc.title)} 
                  ...
                  <button 
                    type="button"
                    onClick={() => onUpdateDocument?.(doc.title)} 
                    className="px-3 py-1 rounded bg-primary/10 text-primary text-xs font-bold hover:bg-primary/20 transition-colors border border-primary/20"
                  >
                    Re-upload
                  </button>
                )}
