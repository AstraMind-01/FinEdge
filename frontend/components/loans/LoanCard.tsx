...
import { Loan, EmiScheduleItem } from '../../types';
import { Home, User, Car, GraduationCap, ChevronDown, ChevronUp, AlertTriangle, ArrowRight } from 'lucide-react';
...
export default function LoanCard({ loan, onPayEmi, onPrepay }: LoanCardProps) {
  const [expanded, setExpanded] = useState(false);

  const repaymentPercent = Math.round((loan.emiPaid / (loan.totalTenure || 1)) * 100);
    ...
    <div className="bg-surface-container-low rounded-2xl border border-white/5 overflow-hidden transition-all hover:border-white/10">
      ...
      <div className="p-6 flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex items-start gap-4 lg:w-[240px] shrink-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-on-surface-variant text-[11px] font-mono mt-1">{loan.accountNumber}</p>
        ...
        <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4 items-center font-mono">
          <div>
        ...
        <div className="flex flex-col items-end gap-3 lg:w-[200px] shrink-0">
          <div className="text-right">
            ...
            <p className="text-on-surface font-bold text-base font-mono">₹{loan.nextEmiAmount.toLocaleString('en-IN')}</p>
            <p className="text-on-surface-variant text-[11px] mt-0.5">{loan.nextEmiDate}</p>
          </div>
          <button 
            type="button"
            onClick={() => onPayEmi && onPayEmi(loan)}
            className="bg-primary text-on-primary text-xs font-bold px-5 py-2.5 rounded-xl hover:shadow-[0_0_15px_rgba(240,180,41,0.4)] transition-all w-full lg:w-auto"
          >
            Pay EMI Now
          </button>
        </div>
      ...
      <div className="px-6 pb-4">
        <div className="flex justify-between text-[11px] text-on-surface-variant mb-1.5 font-mono">
          ...
          <span className="font-bold text-on-surface">{repaymentPercent}%</span>
        </div>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 py-3 border-t border-white/5 text-on-surface-variant hover:text-on-surface text-[12px] font-bold transition-colors hover:bg-surface-container/50"
      >
      ...
      {expanded && (
        <div className="px-6 pb-6 pt-2 border-t border-white/5 animate-fade-in">
          <div className="flex gap-6 flex-wrap text-xs mb-6 font-mono">
            <div>
            ...
            <div>
              ...
              <p className="text-on-surface font-bold mt-1">₹{loan.monthlyEmi.toLocaleString('en-IN')}</p>
            </div>
            <div>
            ...
            <div className="ml-auto flex items-center">
              <button 
                type="button"
                onClick={() => onPrepay && onPrepay(loan)}
                className="text-primary text-xs hover:underline font-bold flex items-center gap-1 bg-primary/10 border border-primary/30 px-3 py-1.5 rounded-lg"
              >
                Make Prepayment <ArrowRight size={12} />
              </button>
            </div>
          </div>
          ...
          <div className="overflow-x-auto rounded-xl border border-white/5">
            <table className="w-full text-xs font-mono">
              ...
              <tbody>
                {loan.emiSchedule.map((emi: EmiScheduleItem, i: number) => (
                  <tr key={i} className={`border-t border-white/5 ${i % 2 === 0 ? 'bg-surface-container-low' : 'bg-surface-container/30'}`}>
                    <td className="px-4 py-3 text-on-surface font-sans font-medium">{emi.month}</td>
                    <td className="px-4 py-3 text-on-surface-variant">{emi.dueDate}</td>
                    <td className="px-4 py-3 text-on-surface text-right font-bold">₹{emi.amount.toLocaleString('en-IN')}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-block px-2 py-0.5 rounded-md text-[10px] font-extrabold uppercase tracking-wider border ${
