...
interface ExploreLoanProductsProps {
  onApplyClick?: (productTitle?: string) => void;
}

export default function ExploreLoanProducts({ onApplyClick }: ExploreLoanProductsProps) {
  const products = [
    {
      title: "Personal Loan",
  ...
  return (
    <div className="mt-8">
      <h3 className="text-base sm:text-lg font-bold text-on-surface mb-4">Explore Loan Products</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {products.map((p, i) => (
          <div 
            key={i} 
            onClick={() => onApplyClick && onApplyClick(p.title)}
            className={`bg-gradient-to-br ${p.bg} rounded-2xl p-5 border ${p.border} hover:border-primary/50 transition-all group cursor-pointer flex flex-col justify-between hover:shadow-[0_8px_25px_rgba(0,0,0,0.4)]`}
          >
            <div>
              ...
              <h4 className="text-on-surface font-bold text-sm mb-1">{p.title}</h4>
              <p className="text-primary text-xs font-bold mb-2">From {p.rate} p.a.</p>
              <p className="text-xs text-on-surface-variant mb-4 leading-relaxed">{p.benefit}</p>
            </div>
