...
import AccountProductModal from '../modals/AccountProductModal';
import { useAccounts } from '../../context/AccountContext';

export default function AccountTypesFooter() {
  const { createFixedDeposit } = useAccounts();
  const [selectedProductId, setSelectedProductId] = useState<"savings" | "current" | "fd" | "rd" | null>(null);
  ...
  const handleOpenAccount = async (type: "SAVINGS" | "CURRENT" | "FIXED_DEPOSIT" | "RECURRING_DEPOSIT") => {
    if (type === "FIXED_DEPOSIT" || type === "RECURRING_DEPOSIT") {
      await createFixedDeposit("1", 25000, 12, 7.25);
    }
  };
      ...
      <div className="flex flex-col gap-4 mt-4 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {types.map((type) => (
            <Card 
              key={type.id} 
              onClick={() => setSelectedProductId(type.id)}
              className={`p-5 flex flex-col gap-3 relative overflow-hidden bg-gradient-to-br ${type.bg} border-outline-variant/10 group cursor-pointer hover:border-outline-variant/30 transition-colors`}
            >
