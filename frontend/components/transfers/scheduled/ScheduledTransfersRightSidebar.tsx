...
import { Calendar, Home, RefreshCw, AlertTriangle, Lightbulb, TrendingUp } from 'lucide-react';
...
export default function ScheduledTransfersRightSidebar({ transfers, onQuickSchedule }: ScheduledTransfersRightSidebarProps) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full lg:w-[320px] xl:w-[380px] shrink-0">
          ...
          <div className="flex flex-col gap-2">
            {failedTransfers.map(t => (
              <div key={t.id} className="flex flex-col gap-1 p-2 bg-surface/50 rounded-lg border border-error/10">
                ...
                <button className="text-[11px] font-bold text-error bg-error/10 px-2 py-1 rounded w-fit mt-1 hover:bg-error/20 transition-colors">
                  Retry Now
                </button>
              </div>
            ))}
          ...
          {upcomingThisWeek.map((t, idx) => (
            <div key={t.id} className="flex gap-4 relative">
              ...
              <div className="flex flex-col gap-1 pb-6 w-full">
                <span className="text-[11px] font-bold text-primary tracking-wider uppercase">{t.nextDate}</span>
                <div className="flex items-center justify-between p-3 bg-surface-container-low rounded-lg border border-outline-variant/10 mt-1">
                  <div className="flex flex-col gap-0.5">
                    <span className="font-semibold text-[13px] text-on-surface">{t.beneficiaryName}</span>
                    <span className="text-[11px] text-on-surface-variant">{t.purpose}</span>
                  </div>
                  <span className="font-bold text-[14px] text-on-surface">{formatCurrency(t.amount)}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <Card className="p-5 flex flex-col gap-4 bg-surface-container shadow-sm border border-outline-variant/10">
        ...
        <div className="flex flex-col gap-2">
          ...
          <button 
            ...
            className="flex items-center justify-between p-3 rounded-xl border border-outline-variant/20 bg-surface hover:border-primary/50 hover:bg-primary/5 transition-all group"
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-surface-container-high flex items-center justify-center text-on-surface-variant group-hover:text-primary transition-colors">
                <Home size={14} />
              </div>
              <span className="text-[13px] font-semibold text-on-surface">Schedule EMI</span>
            </div>
            <span className="text-[18px] text-on-surface-variant opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-2 group-hover:translate-x-0">+</span>
          </button>
      ...
      {/* Tips */}
      <Card className="p-4 flex gap-3 bg-gradient-to-r from-surface-container-high to-surface-container shadow-sm border border-outline-variant/10">
