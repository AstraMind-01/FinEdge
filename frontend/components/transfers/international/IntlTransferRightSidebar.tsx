    ...
    <>
      <div className="flex flex-col gap-6 w-full lg:w-[320px] xl:w-[380px] shrink-0 sticky top-24">
        ...
        <Card className="p-5 flex flex-col gap-4 bg-surface-container shadow-sm border border-outline-variant/10">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex justify-between items-center text-[13px]">
              ...
              <span className="font-medium text-on-surface font-mono">{formatCurrency(inrValue)}</span>
            </div>

            <div className="flex justify-between items-center text-[13px]">
              <span className="text-on-surface-variant">They Receive</span>
              <span className="font-bold text-primary font-mono">{formatCurrency(foreignValue, targetCurrency)}</span>
            </div>

            <div className="flex justify-between items-center text-[13px]">
              <span className="text-on-surface-variant">Exchange Rate</span>
              <span className="font-medium text-on-surface font-mono">1 {targetCurrency} = {formatCurrency(currentRate)}</span>
            </div>

            <div className="flex justify-between items-center text-[13px]">
              <span className="text-on-surface-variant">Transfer Fee</span>
              <span className="font-medium text-on-surface font-mono">{inrValue > 0 ? formatCurrency(transferFee) : '---'}</span>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-[16px] text-on-surface font-mono">{formatCurrency(totalPayable)}</span>
            </div>
          ...
          <div className="flex flex-col gap-2">
            {exchangeRates.map(r => (
              <div key={r.currency} className="flex items-center justify-between p-2 rounded-lg bg-surface border border-outline-variant/10">
                ...
                <div className="flex items-center gap-3">
                  <span className="font-medium text-[13px] text-on-surface font-mono">{formatCurrency(r.rate)}</span>
                  {r.trend === 'up' ? (
        ...
        <Card 
          ...
          className="p-4.5 flex gap-3.5 bg-surface-container-low border border-outline-variant/10 rounded-2xl shadow-sm hover:border-outline-variant/30 hover:bg-surface-high/50 transition-all cursor-pointer group relative z-10"
        >
          <MessageSquareMore size={20} className="text-primary shrink-0 mt-0.5" />
          <div className="flex flex-col gap-1 w-full">
            <span className="text-[13px] font-bold text-on-surface group-hover:text-primary transition-colors">Need Help?</span>
            <p className="text-[12px] text-on-surface-variant leading-snug">
              Questions about exchange rates or documents?{" "}
              <button 
                type="button" 
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="text-primary font-bold hover:underline inline-block text-left"
              >
                Chat with Support
              </button>
            </p>
