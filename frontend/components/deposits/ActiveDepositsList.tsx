...
interface ActiveDepositsListProps {
  deposits?: Deposit[];
  onViewDetails?: (deposit: Deposit) => void;
  onBreakFd?: (deposit: Deposit) => void;
  onPayInstallment?: (deposit: Deposit) => void;
}
...
export default function ActiveDepositsList({
  ...
  onBreakFd,
  onPayInstallment
}: ActiveDepositsListProps) {
  const [deposits, setDeposits] = useState<Deposit[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (propDeposits) {
      setDeposits(propDeposits);
      setLoading(false);
      return;
    }
            ...
            {fds.map(fd => (
              <div key={fd.id} className="bg-surface-container rounded-xl border border-surface-container-highest p-5 group hover:border-primary/50 transition-colors relative">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button onClick={() => onViewDetails?.(fd)} className="text-on-surface-variant hover:text-on-surface p-1 rounded-md hover:bg-surface-container-highest transition-colors">
                ...
                <div className="flex gap-3">
                  <button 
                    ...
                    className="flex-1 border border-surface-container-highest text-on-surface text-sm py-2 rounded-lg hover:bg-surface-container-highest transition-colors font-medium"
                  >
                    View Details
                  </button>
                  <button 
                    type="button"
                    onClick={() => onBreakFd?.(fd)}
                    className="flex-1 bg-surface-container-highest text-on-surface text-sm py-2 rounded-lg hover:bg-surface-container-highest/80 transition-colors font-medium text-error hover:text-error/90"
                  >
                    Break FD
                  </button>
                </div>
            ...
            {rds.map(rd => (
              <div key={rd.id} className="bg-surface-container rounded-xl border border-surface-container-highest p-5 group hover:border-primary/50 transition-colors relative">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button onClick={() => onViewDetails?.(rd)} className="text-on-surface-variant hover:text-on-surface p-1 rounded-md hover:bg-surface-container-highest transition-colors">
                    <MoreVertical size={20} />
                  </button>
                </div>
                ...
                <div className="flex gap-3">
                  <button 
                    type="button"
                    onClick={() => onPayInstallment?.(rd)}
                    className="flex-1 bg-surface-container-highest border border-surface-container-highest text-on-surface text-sm py-2 rounded-lg hover:bg-surface-container-highest/80 transition-colors font-medium flex items-center justify-center gap-2"
                  >
                    Pay Installment
                    <ArrowRight size={14} />
                  </button>
